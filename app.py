...
#     psymptoms = list(request.form.values())
...
def decisiontree():
    ...
    #prediction = DecisionTree.predict(final_features)
    #print(prediction)
    #output=round(prediction[0],2)
    
    
    # return render_template('index.html', prediction_text='the dieseas must be : ${}'.format(output))

@app.route('/randomforest')
def randomforest():
    global inputtest
    predict = RandomForest.predict(inputtest)
    predicted=predict[0]
    for a in range(0,len(disease)):
        if(predicted == a):
            output=disease[a]
...
def naivebayes():
    global inputtest
    predict = NaiveBayes.predict(inputtest)
    predicted=predict[0]
    for a in range(0,len(disease)):
        if(predicted == a):
            output=disease[a]
